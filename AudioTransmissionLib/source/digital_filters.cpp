 ...
 //   for (auto i = 0; i < filter_size_; ++i)
  ////  {
   ...
   // }
    ...
    for (auto i = 1; i<filter_size_; ++i)
    {
      output += tmp*b_coefficients_[i - 1];
      auto tmp2 = memory[i];
      memory[i] = tmp;
      tmp = tmp2;
    }
    output += tmp*b_coefficients_[filter_size_-1];
    auto tmp2 = memory[filter_size_-1];
    memory[filter_size_-1] = tmp;
    tmp = tmp2;
//		ShiftMemory(memory);
  ...
  FirPolyphaseBandpassFilter::FirPolyphaseBandpassFilter(double sample_rate, double pass_band_down, double stop_band_down, double pass_band_up, double stop_band_up, double absorbtion_in_stop_band, int decimation_factor)
    :FirBandPassFilter(sample_rate, pass_band_down, stop_band_down, pass_band_up,stop_band_up,absorbtion_in_stop_band), decimation_factor_(decimation_factor), number_of_polyphase_filters(decimation_factor),
    sample_counter_left_(-1), sample_counter_right_(-1)
  {
    InitDecimatorFilter();
  }

  FirPolyphaseBandpassFilter::~FirPolyphaseBandpassFilter()
  {
  }

  void FirPolyphaseBandpassFilter::CreatePolyphaseFilter(double* tmp_b_coefficients, int i)
  {
    for (int j = 0; j < polyphase_filter_size_; ++j)
    {
      if (i + j*decimation_factor_ < filter_size_)
  ...
  void FirPolyphaseBandpassFilter::InitPolyphaseFilters()
  {
    double* tmp_b_coefficients = new double[polyphase_filter_size_];
    for (int i = 0; i < number_of_polyphase_filters; ++i)
    {
  ...
  void FirPolyphaseBandpassFilter::InitFilterOutpuTables()
  {
    output_left_ = new float[number_of_polyphase_filters];
    output_right_ = new float[number_of_polyphase_filters];
  ...
  void FirPolyphaseBandpassFilter::InitDecimatorFilter()
  {
  ...
  float FirPolyphaseBandpassFilter::FilterOutputLeft(float sample)
  {
    ++sample_counter_left_;
    sample_counter_left_ %= decimation_factor_;
    if (sample_counter_left_ == 0)
  ...
  float FirPolyphaseBandpassFilter::FilterOutputRight(float sample)
  {
    ++sample_counter_right_;
    sample_counter_right_ %= decimation_factor_;
    if (sample_counter_right_ == 0)
  ...
  void FirPolyphaseBandpassFilter::InitMemory()
  {
    for (auto filter : polyphase_filters_)
      filter->ClearMemory();
  }

  float FirPolyphaseBandpassFilter::FilterOutput(float sample, int& sample_counter)
  {
    if (sample_counter == 0)
      return polyphase_filters_[sample_counter]->FilterOutputLeft(sample);
    else
